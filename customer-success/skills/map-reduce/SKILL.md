---
name: map-reduce
description: Run the same analysis or task across multiple targets in parallel, then merge the results. Use when auditing multiple files, reviewing multiple modules, or processing a batch of items.
triggers:
  - audit all of these
  - review every module
  - check all files
...
Split work across parallel agents (map), then combine their results into a single report (reduce).
...
| Targets | Strategy |
|---------|----------|
| 1-2 | Do not use map-reduce. Just do the work. |
